...
export class GameRoom extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    if(url.pathname==="/custom/configure"&&request.method==="POST"){
      ...
      const requestedLimit=Math.trunc(body.config.maxPlayers);const config:CustomConfig={name:safeString(body.config.name,"CUSTOM SERVER",24).replace(/[^a-z0-9 _-]/gi,"").trim()||"CUSTOM SERVER",map:SPAWNS[body.config.map]?body.config.map:"CITY BLOCK",mode:["FFA","TDM","KOTH","CTP","CTF"].includes(body.config.mode)?body.config.mode:"FFA",maxPlayers:requestedLimit===0?0:Math.max(2,Math.min(16,requestedLimit||8))};
