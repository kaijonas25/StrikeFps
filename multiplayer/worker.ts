...
type BotObstacle=[number,number,number,number];
const BOT_OBSTACLES:Record<MultiplayerMap,BotObstacle[]>={
  "CITY BLOCK":[[-31,-29,12,11],[31,-29,12,11],[-31,29,12,11],[31,29,12,11],[-15,-29,4,7],[15,-29,4,7],[-15,29,4,7],[15,29,4,7]],
  "BLACKWOOD FOREST":[[-18,-4,5,4],[19,5,5,4],[-3,19,4,4],[5,-21,5,3],[-28,24,2,2],[27,-25,2,2]],
  "FROSTLINE BASE":[[-22,4,6,5],[22,5,6,5],[0,-17,7,4],[-31,-14,3,3],[31,-14,3,3]],
  "TIDEBREAK BEACH":[[-31,57,6,5],[32,58,6,5],[-43,35,4,4],[43,35,4,4],[-15,-27,4,4],[16,-29,4,4],[0,-28,5,18]],
  "DUSTFALL DESERT":[[-48,42,4,4],[-34,43,5,4],[-17,44,4,5],[17,44,5,4],[34,43,5,4],[49,41,4,5],[-48,24,5,5],[-29,25,4,4],[-11,24,5,5],[12,25,4,4],[30,23,5,5],[49,22,4,5],[-47,3,5,4],[-28,5,5,5],[-9,4,4,4],[10,3,5,5],[30,4,5,4],[49,2,4,5],[-49,-19,4,5],[-31,-19,5,4],[-12,-19,4,5],[11,-18,5,4],[31,-20,4,5],[49,-20,5,4],[-47,-41,5,4],[-30,-42,4,5],[-12,-41,5,4],[13,-42,4,5],[31,-41,5,4],[49,-40,4,5]]
};
...
export class GameRoom extends DurableObject<Env> {
  ...
  private botBlocked(map:MultiplayerMap,x:number,z:number){const radius=.52,boundary=map==="TIDEBREAK BEACH"?58:map==="DUSTFALL DESERT"?58:46;if(Math.abs(x)>boundary||z>boundary||(map!=="TIDEBREAK BEACH"&&z<-boundary)||(map==="TIDEBREAK BEACH"&&z<-48))return true;return BOT_OBSTACLES[map].some(([cx,cz,hx,hz])=>x>cx-hx-radius&&x<cx+hx+radius&&z>cz-hz-radius&&z<cz+hz+radius);}
  private moveBot(map:MultiplayerMap,bot:PlayerState,dx:number,dz:number){let moved=false;if(!this.botBlocked(map,bot.x+dx,bot.z)){bot.x+=dx;moved=true;}if(!this.botBlocked(map,bot.x,bot.z+dz)){bot.z+=dz;moved=true;}return moved;}
  private botLineClear(map:MultiplayerMap,a:[number,number],b:[number,number]){const distance=Math.hypot(b[0]-a[0],b[1]-a[1]),steps=Math.max(1,Math.ceil(distance/1.5));for(let step=1;step<steps;step++){const t=step/steps;if(this.botBlocked(map,a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t))return false;}return true;}
  private pathTo(map:MultiplayerMap,from:[number,number],to:[number,number]){const nodes=BOT_NAV[map],nearest=(point:[number,number])=>nodes.reduce((best,node,index)=>Math.hypot(node[0]-point[0],node[1]-point[1])<Math.hypot(nodes[best][0]-point[0],nodes[best][1]-point[1])?index:best,0),start=nearest(from),goal=nearest(to),queue=[start],came=new Map<number,number>([[start,-1]]);while(queue.length){const current=queue.shift()!;if(current===goal)break;nodes.forEach((node,index)=>{if(!came.has(index)&&Math.hypot(node[0]-nodes[current][0],node[1]-nodes[current][1])<=29&&this.botLineClear(map,nodes[current],node)){came.set(index,current);queue.push(index);}});}const path:[number,number][]=[];let cursor=goal;while(cursor!==-1&&came.has(cursor)){path.unshift(nodes[cursor]);cursor=came.get(cursor)!;}path.push(to);return path;}
