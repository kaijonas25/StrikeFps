...
export function FpsGame() {
  ...
  const [customServerSetup,setCustomServerSetup]=useState(false);
  const [customServerName,setCustomServerName]=useState("MY CUSTOM SERVER");
  const [customServerMap,setCustomServerMap]=useState<Exclude<GameMap,"TEST YARD">>("CITY BLOCK");
  const [customServerMode,setCustomServerMode]=useState<GameMode>("FFA");
  const [customServerMaxPlayers,setCustomServerMaxPlayers]=useState(8);
  ...
  useEffect(() => {
    ...
    if (started && selectedSector !== "TRAINING SECTOR" && MULTIPLAYER_SERVER) {
      ...
      multiplayerSocket.addEventListener("message", (event) => {
        ...
        try {
          ...
          else if (packet.type === "player_health" && packet.id && typeof packet.health === "number" && packet.health > 0) {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if(playerClass==="AIRSTRIKE"&&currentSlot===5){const width=airstrikeBounds.maxX-airstrikeBounds.minX,depth=airstrikeBounds.maxZ-airstrikeBounds.minZ;airstrikeAim.x=THREE.MathUtils.clamp(airstrikeAim.x+e.movementX*width*.0008,airstrikeBounds.minX,airstrikeBounds.maxX);airstrikeAim.y=THREE.MathUtils.clamp(airstrikeAim.y+e.movementY*depth*.0008,airstrikeBounds.minZ,airstrikeBounds.maxZ);return;}
    ...
    const useClassItem = () => {
      const now = performance.now();
      if (now < classAbilityReadyAt) return;
      const target = playerClass==="MORTAR"?mortarTarget():playerClass==="AIRSTRIKE"?airstrikeTarget():classTarget();
      if (playerClass === "MORTAR") {
        ...
        callAirstrike(target.x,target.z);airstrikeZone.visible=false;
        ...
        const turretHead=new THREE.Group();turretHead.position.y=.72;sentry.add(turretHead);
        const armoredBody=sentryPart(new THREE.BoxGeometry(.78,.48,.72),0x35453d,.48,.62);armoredBody.position.z=.02;turretHead.add(armoredBody);
        const slopedTop=sentryPart(new THREE.CylinderGeometry(.43,.32,.19,4),0x425248,.52,.58);slopedTop.rotation.y=Math.PI/4;slopedTop.position.y=.32;turretHead.add(slopedTop);
        const frontShield=sentryPart(new THREE.BoxGeometry(.72,.38,.11),0x26322e,.5,.68);frontShield.position.set(0,.03,-.42);turretHead.add(frontShield);
        const ammoBox=sentryPart(new THREE.BoxGeometry(.34,.42,.48),0x4b563e,.78,.38);ammoBox.position.set(.57,-.02,.08);turretHead.add(ammoBox);
        for(let link=0;link<6;link++){const belt=sentryPart(new THREE.BoxGeometry(.045,.035,.1),0xb89a55,.42,.72);belt.position.set(.38-link*.055,.02,-.22-link*.045);belt.rotation.x=-.35;turretHead.add(belt);}
        const barrelSlides:THREE.Group[]=[];const muzzles:THREE.Object3D[]=[];
        [-.17,.17].forEach((x)=>{const slide=new THREE.Group();slide.position.set(x,.04,-.38);turretHead.add(slide);barrelSlides.push(slide);const shroud=sentryPart(new THREE.CylinderGeometry(.075,.09,.72,14),0x171d1c,.4,.82);shroud.rotation.x=Math.PI/2;shroud.position.z=-.31;slide.add(shroud);const barrel=sentryPart(new THREE.CylinderGeometry(.032,.042,.72,12),0x0c1111,.28,.92);barrel.rotation.x=Math.PI/2;barrel.position.z=-.92;slide.add(barrel);const brake=sentryPart(new THREE.CylinderGeometry(.065,.055,.18,12),0x111716,.32,.9);brake.rotation.x=Math.PI/2;brake.position.z=-1.32;slide.add(brake);const muzzleAnchor=new THREE.Object3D();muzzleAnchor.position.z=-1.43;slide.add(muzzleAnchor);muzzles.push(muzzleAnchor);});
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      if (currentSlot === 5) { if(playerClass==="MORTAR"&&!mortarDeployed)return; classAiming = true; classTargetMarker.visible = playerClass!=="MORTAR"&&playerClass!=="AIRSTRIKE"; if(playerClass==="MORTAR")trajectory.visible=true;if(playerClass==="AIRSTRIKE")airstrikeZone.visible=true; return; }
      if (currentSlot === 3) {
    ...
    const onMouseUp = (e: MouseEvent) => {
      if (e.button === 0) {
        triggerHeld = false;
        droneTrigger = false;
        if (classAiming) { classAiming = false; classTargetMarker.visible = false; if(playerClass==="MORTAR")trajectory.visible=false;if(playerClass==="AIRSTRIKE")airstrikeZone.visible=false; useClassItem(); }
    ...
    const animate = () => {
      ...
      localPlayer.visible = isThirdPerson || Boolean(activeDrone) || ((playerClass==="MORTAR"||playerClass==="AIRSTRIKE")&&currentSlot===5);
  ...
  return (
    <main className={`game-shell${!started ? " game-menu" : ""}`}>
      ...
      {started&&selectedSector==="CUSTOM SERVER"&&<div className="custom-code-hud"><small>CUSTOM SERVER CODE</small><strong>{customServerCode}</strong></div>}
        ...
        <section className="menu-card">
          ...
          {(!started && menuPage === "HOME" && serverBrowserOpen) && <div className="server-browser">
            ...
            <div className="server-list">
              ...
              <div className={`custom-server-card${customServerSetup?" configuring":""}`}>
                <header><i>CS</i><span><b>CUSTOM SERVER</b><small>CREATE A PRIVATE ROOM OR JOIN WITH A SIX-CHARACTER CODE</small></span></header>
                {!customServerSetup&&<><div><input value={customServerCode} maxLength={6} placeholder="SERVER CODE" aria-label="Custom server code" onChange={(event)=>{setCustomServerCode(event.target.value.toUpperCase().replace(/[^A-Z2-9]/g,"").slice(0,6));setCustomServerStatus("");}} />
                ...
                <button disabled={!auth.currentUser} onClick={()=>{if(!auth.currentUser){setCustomServerStatus("SIGN IN TO CREATE A SERVER");return;}setCustomServerSetup(true);setCustomServerStatus("");}}>CREATE SERVER</button></div>
                <small>{customServerStatus||(!auth.currentUser?"SIGN IN REQUIRED TO CREATE · CODES CAN STILL BE JOINED":"CONFIGURE YOUR OWN PRIVATE MATCH")}</small></>}
                {customServerSetup&&<div className="custom-server-setup"><label>SERVER NAME<input value={customServerName} maxLength={24} onChange={(event)=>setCustomServerName(event.target.value.toUpperCase())}/></label><label>STARTING MAP<select value={customServerMap} onChange={(event)=>setCustomServerMap(event.target.value as Exclude<GameMap,"TEST YARD">)}>{(["CITY BLOCK","BLACKWOOD FOREST","FROSTLINE BASE","TIDEBREAK BEACH","DUSTFALL DESERT"] as const).map((map)=><option key={map}>{map}</option>)}</select></label><label>GAME MODE<select value={customServerMode} onChange={(event)=>setCustomServerMode(event.target.value as GameMode)}>{(["FFA","TDM","KOTH","CTP","CTF"] as const).map((mode)=><option key={mode} value={mode}>{mode==="CTP"?"CAPTURE POINTS":mode==="CTF"?"CAPTURE THE FLAG":mode==="KOTH"?"KING OF THE HILL":mode==="TDM"?"TEAM DEATHMATCH":"FREE FOR ALL"}</option>)}</select></label><label>PLAYER LIMIT<select value={customServerMaxPlayers} onChange={(event)=>setCustomServerMaxPlayers(Number(event.target.value))}><option value={0}>NO PLAYER LIMIT</option>{Array.from({length:15},(_,index)=>index+2).map((limit)=><option key={limit} value={limit}>{limit} PLAYERS</option>)}</select></label><div className="custom-server-actions"><button onClick={()=>setCustomServerSetup(false)}>CANCEL</button><button onClick={async()=>{if(!auth.currentUser)return;setCustomServerStatus("STARTING SERVER…");try{const token=await auth.currentUser.getIdToken();const response=await fetch(`${MULTIPLAYER_SERVER}/custom/create`,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({name:customServerName,map:customServerMap,mode:customServerMode,maxPlayers:customServerMaxPlayers})});const data=await response.json() as {code?:string;error?:string};if(!response.ok||!data.code)throw new Error(data.error||"CREATE FAILED");setCustomServerCode(data.code);setSelectedSector("CUSTOM SERVER");setServerBrowserOpen(false);setSelectedMap(customServerMap);setMatchPhase("connecting");setMapVotes(0);setModeVotes(0);setHasVoted(false);setHasModeVoted(false);setMatchEndsAt(0);setHealth(maximumHealth);setStarted(true);setSessionId((id)=>id+1);}catch(error){setCustomServerStatus(error instanceof Error?error.message:"CREATE FAILED");}}}>START SERVER</button></div><small>{customServerStatus||"THE SERVER CODE IS CREATED ONLY AFTER STARTING"}</small></div>}
              </div>
            </div>
          </div>}
